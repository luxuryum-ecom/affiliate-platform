/**
 * Config Playwright — Réserve R1 : never-refuse submitWholesaleOrder via le VRAI chemin.
 *
 ...
 * Pré-requis :
 *   1. supabase start
 *   2. npx playwright test --config=playwright.r1.config.ts
...
const PORT = 3301 // port dédié R1 — n'entre pas en collision avec les autres suites
const BASE_URL = `http://localhost:${PORT}`

export default defineConfig({
  testDir: './e2e',
  testMatch: /etape7-r1-never-refuse\.spec\.ts/,
  fullyParallel: false,
  forbidOnly: false,
  retries: 0,
  workers: 1,
  reporter: [['list'], ['html', { open: 'never', outputFolder: 'playwright-report-r1' }]],
  timeout: 180_000,
