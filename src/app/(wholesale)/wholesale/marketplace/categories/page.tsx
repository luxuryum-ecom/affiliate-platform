...
// pleine, chacune ciblant le marketplace filtré par catégorie (?category=).
...
export default async function WholesaleCategoriesPage() {
  ...
  const cards: CategoryCardData[] = PRODUCT_CATEGORIES.map((cat) => ({
    ...
    href: `/wholesale/marketplace?category=${encodeURIComponent(cat)}`,
  ...
  return (
    <div className="theme-dark bg-bg text-foreground min-h-screen">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="mt-6">
          <Link href="/wholesale/marketplace" className="text-sm text-gold-400 hover:underline">
            ← {t('categoriesBack')}
          </Link>
        </div>
      </main>
