/**
 * Config Playwright dédiée aux captures du portail livreur cloisonné
 * `/courier/scan` (module Livreurs, Lot B) en FR/AR, viewport mobile.
 *
 ...
 *
 * Pré-requis : « supabase start » + « node scripts/seed-lotb-captures-local.mjs --seed »
 */
...
const PORT = 3302 // port dédié — n'entre pas en collision avec 3000/3200/3300/3301
const BASE_URL = `http://localhost:${PORT}`

export default defineConfig({
  testDir: './e2e',
  testMatch: /lotb-captures\.spec\.ts/,
  fullyParallel: false,
  forbidOnly: false,
  retries: 0,
  workers: 1,
  reporter: [['list'], ['html', { open: 'never', outputFolder: 'playwright-report-lotb' }]],
  timeout: 120_000,
