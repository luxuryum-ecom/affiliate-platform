...
2. 🔴 **CHANTIER MONEY — purge `parseFloat` sur l'argent** (objectif Abdou « **zéro parseFloat sur l'argent nulle part** »). Découverte : surface bien plus large que les 7 sites d'`orders.ts` (~12 fichiers). Stratégie tranchée : **(A)** input → `parseMoneyInput` (string verbatim) ; **(B)** calcul → **centimes entiers** `Math.round(x*100)*qty/100` (validé @finance) ; **(C)** taux FX & marges % → helpers séparés `parseRateInput`/`parsePercentInput`, **lots dédiés distincts** (money.ts les corromprait) ; **(D)** preview client → dernier lot, sans @finance.
   ...
   - ⏳ **RESTE** (découpage affiné) :
     - **M7a `supplier-products.ts`** — `tier_${i}_price` (26) + `price_source` (85) = montants (A). ⚠️ vérifier que `price_source` n'alimente pas un calcul FX avant conversion. Marge % (184) = hors périmètre (lot %).
     - **M7b `supplier-payout.ts` + `quote-requests.ts`** — montants (A) ; isoler les taux FX (`quote-requests` 107/130/266) → lot FX.
     - **🟧 M-products `products.ts` — LOT DÉLICAT, décision Abdou requise** : `purchase_price` (78) alimente `factory_cost = round(purchase × taux_FX)` (125) → entanglement montant↔FX↔calcul. Refactorer 125 en centimes entiers/`Math.round` **changerait potentiellement la valeur de `factory_cost_mad`** (comme M-commission). Fees (108-110), `factory_cost` direct (140-141), `sell_price` (146), `estimated_import` (209) = montants A simples ; `exchange_rate` (86) = FX ; `margin_value` (103) = %. À cadrer ensemble car entremêlés.
     - **M8 preview client** (`product-form.tsx` 41/218-239, `order-status-form.tsx` 49, `admin/quote-requests/[id]/page.tsx` 68) = D, affichage, sans @finance.
     - **Lots FX/% dédiés** : `fx.ts` (22), `exchange_rate`/FX computations (products 86/125, quote-requests 107/130/266), marges % (products 103, supplier-products 184) → helpers `parseRateInput`/`parsePercentInput`.
   - ⚠️ Chaque lot money = `@finance` par site, validation Abdou si un calcul **change de résultat**. Cf. [[project-money-no-parsefloat]].
   ▶️ **REPRISE = M7a `supplier-products.ts`** (montants simples), puis M7b, puis cadrer **M-products** (décision Abdou sur l'arrondi `factory_cost`), puis M8 + lots FX/%. Commits poussés jusqu'à `405cf45`.
