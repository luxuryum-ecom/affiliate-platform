/**
 * Config Playwright dédiée aux captures des écrans admin du module Livreurs
 * Lot D — `/admin/couriers/[id]` (Tournées + Retours) et `/admin/couriers/pickup`
 * (scan ramassage dépôt) — en FR/AR.
 *
 ...
 *
 * Pré-requis : « supabase start » + « node scripts/seed-lotd-captures-local.mjs --seed »
 */
...
const PORT = 3304 // port dédié — n'entre pas en collision avec 3000/3200/3300-3303
const BASE_URL = `http://localhost:${PORT}`

export default defineConfig({
  testDir: './e2e',
  testMatch: /lotd-captures\.spec\.ts/,
  fullyParallel: false,
  forbidOnly: false,
  retries: 0,
  workers: 1,
  reporter: [['list'], ['html', { open: 'never', outputFolder: 'playwright-report-lotd' }]],
  timeout: 120_000,
