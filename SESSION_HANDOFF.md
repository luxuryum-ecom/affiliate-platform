...
> **Lot 3 — Niche déclarée signup grossiste** (merge, branche `feat/niche-declared-signup`) : **migration 117** (`profiles.declared_niche` + trigger `handle_new_user` étendu, allowlist rôle mig 090 inchangée), champ `<select>` de catégories LOCALISÉES au signup grossiste (facultatif, i18n FR/AR/EN, validé `isValidCategory`), fallback COLD-START dans marketplace (`niche = comportement ?? declared_niche` ; grille 🔒 intouchée, uniquement boost tri + bannière existants). Test intégration LOCAL 4/4. **⚠️ Migration 117 = APPLIQUÉE EN LOCAL SEULEMENT → à appliquer en PROD (lockstep : après push Abdou + déploiement Vercel, Claude applique via `backups/.db_password`).**
> **Lot 4 — Qualité photo IA (C2)** — **PAS MERGÉ** (branche `feat/c2-photo-quality-ai`, `aa0d5ec`) : `photo_issue` ajouté à l'extraction Haiku (MÊME appel, 0 coût IA) + `photo-quality.ts` (fail-open → 'ok' si incertain) + câblage `ingest.ts` : NON-PRODUIT → aucune fiche + guide ; FLOU → fiche créée + signal `blurry_photo` + revue admin forcée + invitation photo nette. Messages bot 4 langues 🔒 **wording en attente de validation Abdou (capture RTL demandée)**. 9 tests. Pas de migration. **GO merge à donner après validation des textes.**
> **4 checks verts sur `main` combiné (Lots 1+2+3) : tsc 0 · build · vitest 629 · smoke 16.** Détail par lot : mémoire `project-4lots-awaiting-go`.
>
