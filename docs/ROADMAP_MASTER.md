...
- **🚫 PRÉ-BETA BLOQUANT — BACKUPS PROD QUOTIDIENS OBLIGATOIRES** : **avant tout vrai client**, mettre en place des **sauvegardes automatiques quotidiennes de la base PROD** (Supabase Pro = PITR/daily backups, **OU** cron `pg_dump` horodaté hors-repo). **Ne PAS lancer la beta sans ça.** Aujourd'hui la prod ne contient que des **données de test** (backup manuel non critique), mais dès le 1ᵉʳ client réel l'absence de backup = risque de perte de données inacceptable. **Condition bloquante de go-live.**
...
**🔄 F2 — LOT B Étape 7 : CODÉE + AUDITÉE, branche `feat/etape7-bascule-stock`, NON mergée, EN ATTENTE GO ABDOU (cutover).** Le stock variante est désormais la **source de vérité d'audit** : **C-1** écriture primaire `product_variants.stock_count` + **C-2** `balance_after` ledger sur la variante (mig 105), avec **DOUBLE-ÉCRITURE MAINTENUE** (`products.stock_count` = cache dérivé, MÊME delta → I1 préservé → **réversible**). On NE coupe PAS encore l'ancien compteur (= 7.D, seul vrai non-retour, GO séparé). 7.A (affichage variante) + 7.B (lecteurs décision → variant_id, Q2 never-refuse grossiste) inclus. **Fix grossiste pré-existant** (mig 091 lockstep : page panier + `submitWholesaleOrder` lisaient `products` en direct → crash ; repointés vue redacted / service_role pour le coût, calcul inchangé). **@finance GO + @security GO + 4 checks verts** (tsc 0 / build / vitest 318 / smoke 16/16). Preuves LOCAL : C-1/C-2 47/47, affichage 21/21, anti-double-comptage 2/2, never-refuse fonctionnel+DB, money 400.00/99.99. **Détail → `ETAT_SYSTEME.md`.** Reste post-GO : merge + `db push` prod (mig 105), puis plus tard **7.D** (désactiver la double-écriture) en lot séparé. Ticket : stabiliser l'e2e never-refuse (harnais instable, code sain). **En parallèle : F4 — nettoyage sécurité** (rotation `SUPABASE_SERVICE_ROLE_KEY`, compte fournisseur smoke prod) + **backups prod quotidiens (pré-beta bloquant, cf. §2)**. C4 pack grossiste = reporté.
