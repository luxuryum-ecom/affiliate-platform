...
2. 🔴 **PRIORITAIRE — CHANTIER MONEY DÉDIÉ : purge des 7 `parseFloat` restants sur l'argent** (objectif Abdou « **zéro parseFloat sur de l'argent nulle part** », non négociable). À faire **JUSTE APRÈS le Lot 4, AVANT le staging**. Sites dans `src/app/actions/orders.ts` : flux COD/commission/pricing `147`/`271`/`337`/`525`/`536`/`640`/`651` + dette déjà ticketée `707-709` (coûts wholesale). Convertir vers le helper `src/lib/money.ts` (chaîne décimale verbatim). ⚠️ Cœur commission/COD → **circuit `@finance` + `@security` PAR SITE**, lots petits, validation Abdou avant chaque commit. Cf. [[project-money-no-parsefloat]].
