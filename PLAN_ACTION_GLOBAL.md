...
- ✅ Audit RLS ciblé tables gros/fournisseur (périmètre réduit du futur B1 complet). **AUDIT FAIT 2026-07-05** (@security, 6 findings) **+ correctif fuites INTER-ACTEURS MERGÉ `main` `--no-ff` `df8b1ce`** (mig 115, @finance🟢 @security🟢 @tester🟢, NON poussé — Abdou pousse + SQL). **Fermé** : C1/C2 (prix source USD fournisseur lisible par grossiste) + E2 (coût d'usine `products` → staff-only ; 091 déjà active en prod). **⏸️ Reste à faire (lot dédié @finance)** : E1 (acheteur voit marge sur SA commande) + M1 (fournisseur voit marge sur SA fiche) — fuites sur ses propres lignes, entremêlées aux writes (règle #3). M2 → backlog.
