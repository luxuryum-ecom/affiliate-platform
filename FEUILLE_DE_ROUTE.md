...
2. 🔴 **CHANTIER MONEY — purge `parseFloat` sur l'argent** (objectif Abdou « **zéro parseFloat sur l'argent nulle part** »). Découverte : surface bien plus large que les 7 sites d'`orders.ts` (~12 fichiers). Stratégie tranchée : **(A)** input → `parseMoneyInput` (string verbatim) ; **(B)** calcul → **centimes entiers** `Math.round(x*100)*qty/100` (validé @finance) ; **(C)** taux FX & marges % → helpers séparés `parseRateInput`/`parsePercentInput`, **lots dédiés distincts** (money.ts les corromprait) ; **(D)** preview client → dernier lot, sans @finance.
   - ✅ **FAIT + audité @finance GO + commité** (session 2026-06-13) : **M1** `createAffiliateOrder` (commits `3ab61a0`), **M2-M4** `placeOrder`/`createWholesaleOrderFromCart`/`submitWholesaleOrder`/`updateWholesaleOrderCosts` (`76e8d3f`), **M-commission** `calculateNetAffiliateCommission` half-up (`5e75999`, arbitrage Abdou : corrige sous-paiement affilié 1ct). **`orders.ts` + `utils.ts` = zéro parseFloat sur argent.**
   - ⏳ **RESTE** : **M5** `cities.ts`/`logistics.ts` (frais config, A) → **M6** `products.ts`/`supplier-products.ts`/`tariffs.ts`/`affiliate-prices.ts` (montants, A — hors marges %) → **M7** `supplier-payout.ts`/`quote-requests.ts` (A, isoler les taux FX) → **M8** preview client (`product-form.tsx`…, D, sans @finance). Puis **lots FX/% dédiés** (`fx.ts`, marges % de products/supplier-products) avec nouveaux helpers. ⚠️ Chaque lot money = circuit `@finance` par site, validation Abdou si un calcul **change de résultat**. Cf. [[project-money-no-parsefloat]].
   ▶️ **REPRISE PROCHAINE SESSION (FRAÎCHE) = LOT M5** (`cities.ts`/`logistics.ts`, frais config, catégorie A). Puis M6 → M7 → M8 → lots FX/marges % dédiés. `@finance` par site, validation Abdou si un calcul change de résultat. Les commits M1-M4 + M-commission sont **poussés** (jusqu'à `6764065`).
