...
- **LOT 3** — Lien fournisseur : `supplier_id` + réponse dispo/délai (vue redacted, sans PII acheteur) + UI fournisseur. **+ traiter IMP-2/IMP-3 ci-dessus.** `@backend-db`+`@frontend` → audit `@security`.
  ...
  - **LOT 3b** *(à faire)* — UI fournisseur (liste commandes redacted + form réponse dispo/délai) + UI admin assignation fournisseur + i18n des 5 clés `errors.*` (supplier_not_found/supplier_id_required/forbidden_supplier_only/invalid_supplier_response/invalid_lead_time) FR/AR/EN. **+ traiter IMP-2** (repli agents scopé owner). `@frontend` → audit `@security`.
