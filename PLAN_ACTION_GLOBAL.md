...
- ✅ Audit RLS ciblé tables gros/fournisseur (périmètre réduit du futur B1 complet). **AUDIT FAIT 2026-07-05** (@security, 6 findings) **+ correctif fuites INTER-ACTEURS MERGÉ `main` `df8b1ce` + migration 115 APPLIQUÉE EN PROD 2026-07-05** (@finance🟢 @security🟢 @tester🟢 ; vérifié AVANT/APRÈS : 0 policy `wholesaler read` restante). **Fermé EN PROD** : C1/C2 (prix source USD fournisseur lisible par grossiste) + E2 (coût d'usine `products` → staff-only ; 091 déjà active). **✅ E1 + M1 FERMÉS (lot `fix/e1-m1-marge-propres-lignes`, mergé `main` `--no-ff` `a43491d`, 2026-07-06)** : E1 (acheteur voit marge sur SA commande via `wholesale_orders`) + M1 (fournisseur voit marge sur SA fiche via `supplier_products`) → SELECT base réservé staff + vues redacted owner (`wholesale_orders_buyer_read` existante / `supplier_products_owner_read` nouvelle, sans colonne de marge). Pattern mig 060/115. `submitWholesaleOrder` insert/rollback via service_role ; `getProductLimitStatus` garde self-only (IDOR fermé en bonus). @finance🟢 (0 calcul touché) @security🟢 (0 P0/P1). Test LOCAL 7/7, 4 checks verts (vitest 598). **✅ Migration 116 APPLIQUÉE EN PROD 2026-07-06** (pooler, transaction atomique, après push + déploiement Vercel — lockstep ; vérifié AVANT/APRÈS : branche `buyer_id` de `wholesale_orders: read` disparue, « supplier read own » → `admin read`, vue owner créée sans marge). **E1 + M1 FERMÉS EN PROD.** M2 (mutation paliers post-approbation, indicatif) → backlog. **Bloc 1A BOUCLÉ.**
  - **⚠️ RÉGRESSION mig 116 DÉCOUVERTE & CORRIGÉE 2026-07-07** (mergé `main` `--no-ff` `32d2ee7`) : rendre `wholesale_orders` staff-only a cassé 4 policies acheteur (subquery `EXISTS` sur la table base) → l'acheteur ne voyait plus ses **lignes de commande** (« 0 article » sur toutes les commandes prod), facture non itemisée, réassort cassé. **Fix (mig 120)** : fonction `is_my_wholesale_order(uuid)` SECURITY DEFINER (booléen d'appartenance, aucune marge) + 4 policies réécrites + 3 consommateurs repointés sur la vue redacted `products_catalog_read` (null-safe, sinon crash `products` base null). **@finance🟢 (E1 reste fermé) @security🟢** (isolation cross-buyer prouvée). Test LOCAL 5/5, 4 checks verts (vitest 660). **✅ Migration 120 APPLIQUÉE EN PROD 2026-07-07** (pooler, transaction atomique, lockstep, vérifié AVANT/APRÈS ; E1 intact). Correctif PRIORITAIRE LIVE.
...
- ✅ AM-1 RÉASSORT 1-CLIC (« recommander ma dernière commande »). **FAIT 2026-07-07, mergé `main` `--no-ff`** — bouton sur `/wholesale/orders` → recharge le panier (helper pur `reorder.ts`, action `reorderLastOrder`, aucun prix copié → le panier recalcule) + bannière bilan. i18n FR/AR/EN. Non-financier, RLS own-rows. 7 tests. Pas de migration.
...
- ✅ V5 ALERTE PRIX / LISTE DE SUIVI (prix baisse). **FAIT 2026-07-07, mergé `main` `--no-ff`** — bouton « Suivre le prix » sur `/wholesale/marketplace/[id]` ; à la baisse du prix de gros public, notification `price_drop` dans la cloche (via trigger DB SECURITY DEFINER `notify_price_drop`, réutilise `notifications` mig 076). **@security 🟢** (0 P0/P1 ; ne diffuse que le prix public, jamais la marge). i18n FR/AR/EN. 4 tests. **✅ Migration 118 APPLIQUÉE EN PROD 2026-07-07** (pooler, transaction atomique, lockstep ; table+RLS 3 policies+trigger SECURITY DEFINER vérifiés). *(canal WhatsApp = plus tard, dépend d'AM-8 Business API.)*
...
- 🔄 AM-10 PWA installable (icône écran d'accueil). **CODÉ 2026-07-09, branche `feat/am10-pwa`, EN ATTENTE GO merge.** Anti-fausse-dette vérifié (aucun manifest/SW/next-pwa préexistant). Livré : `src/app/manifest.ts` (Next 15, `/manifest.webmanifest` standalone, thème Noir+Or 🔒), `public/sw.js` (SW minimal SÛR — 0 cache de contenu dynamique/auth, repli hors-ligne only), `public/offline.html` (FR/AR/EN + RTL), 4 icônes placeholder « A » doré vectoriel sur noir (`public/icons/` : 192/512/maskable-512/apple-touch-180 ; icône temporaire, définitive = fichier logo Abdou Baba du designer — non bloquant), `src/components/pwa/install-prompt.tsx` (bannière `beforeinstallprompt` + hint iOS, i18n `pwa` FR/AR/EN, RTL), câblage `layout.tsx` (appleWebApp + viewport theme-color). **Preuve installabilité navigateur réelle (Chromium/CDP)** : `link rel=manifest` injecté, service worker **activated**, manifest parse errors **[]**, **installability errors [] = Chrome le juge installable**. Bannière rendue FR + AR/RTL (captures). Non-financier, **pas de migration**, 4 checks verts (tsc 0 · build · vitest 660 · smoke 16).
...
- 🟢 B2 — GRAND LIVRE double entrée : comptes internes dont « cash en transit chez livreur X »,
  append-only, idempotent, somme=0, rapprochement nocturne + alertes. **MVP FAIT 2026-07-09** (branche `feat/b2-double-entry-ledger`, EN ATTENTE GO merge) — **migration 121** : 3 tables (`ledger_accounts` chart of accounts / `ledger_transactions` / `ledger_postings`), signe débit+/crédit−, **contrainte somme=0 garantie en base** (RPC valide + CONSTRAINT TRIGGER déféré au COMMIT), immuabilité (UPDATE/DELETE/TRUNCATE bloqués), idempotence (clé unique + anti-course), RLS deny-défaut (staff + party own), **10 comptes internes** dont `cash_in_transit_courier` (LE point de fuite), `platform_margin_income`, `affiliate_commission_payable`, `supplier_payable`, `platform_cash`. RPC `record_ledger_transaction` (garde admin/service_role, mono-devise MAD MVP). **COEXISTE avec `ledger_entries` (048) SANS le toucher** (l'affilié = un compte, pas de double-comptage). @finance cadré · **@security 🟢 (0 P0/P1**, P2 durcis P2-1/P2-3, P2-2/P2-4 reportés mig 122). Test LOCAL 9/9 (cycle COD boucle à 0, fuite livreur tracée, immuabilité, idempotence, numeric). 4 checks verts (vitest 669). **RESTE (mig 122+, circuit @finance dédié)** : branchement AUTO sur triggers commande (delivered → txn `cod_collected`, retour → contre-passation), snapshots `factory_cost_mad`/`platform_margin_mad` sur `orders` (absents), écran remise livreur + réconciliation, invariant de réconciliation avec 048.
...
- ✅ B8 — Suppression compte RGPD. **FAIT 2026-07-07, mergé `main` `--no-ff`** — anonymisation (pas de DELETE physique, verrou FK/audit) : PII profil vidée (dont `bank_account`) + email auth anonymisé + ban connexion + `signOut` global ; commandes gardent `buyer_id` (intégrité comptable). Action `requestAccountDeletion` (service_role borné à user.id, auth-first, admin ne peut pas s'auto-supprimer, confirmation obligatoire), garde `signIn` sur statut `deleted`. Zone dangereuse `/wholesale/account`. **@security 🟢 après P1** (bank_account/sessions/ordre auth corrigés). i18n FR/AR/EN. 6 tests. **✅ Migration 119 APPLIQUÉE EN PROD 2026-07-07** (pooler, transaction atomique, lockstep ; statut `deleted` accepté + `anonymized_at` créée, 0 profil modifié).
...
- ✅ C2 BRIQUE 2 — contrôle qualité photo IA à la réception (textes validés Abdou 🔒). **FAIT 2026-07-07, mergé `main` `--no-ff`** — `photo_issue` ajouté à l'extraction Haiku (même appel, 0 coût IA) + `photo-quality.ts` (fail-open → 'ok' si incertain) + câblage `ingest.ts` : NON-PRODUIT → aucune fiche + guide ; FLOU → fiche créée + signal `blurry_photo` + revue admin forcée + invitation photo nette. **Messages bot 4 langues (FR/EN/AR-fusha/darija) VALIDÉS par Abdou** (rendu RTL vérifié sur capture `Desktop/rendu-photo-messages.png`). 9 tests. Pas de migration.
