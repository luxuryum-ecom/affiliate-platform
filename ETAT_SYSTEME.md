...
- **LOT « Navigation marketplace » sur `feat/navigation-marketplace`** (2026-06-21, **affichage pur, zéro argent, zéro migration**) — **construit + 4 checks verts (tsc 0 / build / 239 tests / smoke 20/20), PAS mergé** : (1) **`CategoryRail` ajouté sur `/wholesale/marketplace`** (réutilisation directe, chips serveur ciblant le `?category=` existant) ; (2) **section « cartes-rayons » EN HAUT** du marketplace (carrousel horizontal mobile / grille desktop) + **route bonus `/wholesale/marketplace/categories`** (composant `CategoryShowcase`/`CategoryCard`, Server Component pur, fallback emoji CSS) ; (3) **12 images self-hostées WebP** `public/categories/` (Unsplash libre de droit, `CREDITS.md`) + `CATEGORY_IMAGES` dans `taxonomy.ts`. **100 % ADDITIF** : grille produit / dropdown / filtres / CTA **intouchés** (diff marketplace = 66 ins / 1 del = import ; bas de page identique avant/après — grilles dimensions identiques FR/AR/EN mobile+desktop, captures Playwright). i18n FR/AR/EN + RTL vérifiés. **PHASE D : attente accord visuel avant merge.**
