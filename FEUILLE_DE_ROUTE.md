...
> ▶️ **REPRISE PROCHAINE SESSION = LOT 3c** (IMP-3 : atomicité `update`+`insert historique` + rollback réservation partielle → RPC Postgres transactionnelle ; `@backend-db` → audit `@security`). Détail dans la sous-section LOT 3 plus bas. **FAIT à ce jour** : Lots 1, 2 (Deliveroo) + chantier Transport T0→T3:b **poussés** ; **Lot 3a** (lien fournisseur DB + fix sécu `060`) **poussé** ; **Lot 3b** (UI fournisseur + UI admin assignation + i18n + IMP-2) **commité local** (`52e6ff6`/`3d9a809`/`4222c96`, audit `@security` GO, **pas encore poussé**). **Reste** : Lot 3c (IMP-3 atomicité). Migrations appliquées jusqu'à `060`.
...
- **LOT 3** — Lien fournisseur : `supplier_id` + réponse dispo/délai (vue redacted, sans PII acheteur) + UI fournisseur. **+ traiter IMP-2/IMP-3 ci-dessus.** `@backend-db`+`@frontend` → audit `@security`.
  ...
  - **LOT 3c** *(à faire)* — IMP-3 : atomicité `update`+`insert historique` + rollback réservation partielle → RPC Postgres transactionnelle. `@backend-db` → audit `@security`.
