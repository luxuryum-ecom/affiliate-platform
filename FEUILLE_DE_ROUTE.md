...
> ▶️ **REPRISE PROCHAINE SESSION = LOT 4** (Logistique flexible `logistics_mode` + états `picked_up`/`dispatched` ; ⚠️ si coût-ramassage entre dans la marge → bascule circuit `@finance`). Détail dans la sous-section LOT 3/PLAN PAR LOTS plus bas. **✅ LOT 3 COMPLET (a+b+c) — lien fournisseur + UI + atomicité** : Lots 1, 2 (Deliveroo) + Transport T0→T3:b **poussés** ; **Lot 3a** (DB lien fournisseur + fix sécu `060`) **poussé** ; **Lot 3b** (UI fournisseur + UI admin assignation + i18n + IMP-2) + **Lot 3c** (IMP-3 atomicité RPC, migration `061`) **commités+poussés** (`52e6ff6`/`3d9a809`/`4222c96`/`274fb1a`, audits `@security` GO). Migrations appliquées jusqu'à `061`. IMP-1/IMP-2/IMP-3 = **tous résolus**.
...
- **LOT 4** — Logistique flexible (`logistics_mode`) + états picked_up/dispatched. **CHANTIER FINANCIER → circuit `@finance` complet OBLIGATOIRE** (plan `@architect` → audit `@finance` → validation Abdou → code).
  ...
  - **Plan d'exécution (chaque lot financier → audit `@finance` + `@security` AVANT commit)** : **4.1** migration `062` (colonnes + CHECK durci C2 + ledger dédié C1/C3 event-based + RLS deny + triggers anti-mutation + types TS). **4.2** action serveur + zod décimal strict (C4, zéro parseFloat) + RPC écriture ledger idempotente (SECURITY DEFINER) + clés `errors.*` i18n. **4.3** UI form par commande (3 cas, strings pré-résolues) + suivi encaissement + i18n FR/AR/EN.
