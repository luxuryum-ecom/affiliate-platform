...
> **⚠️ Migration 120 = APPLIQUÉE EN LOCAL SEULEMENT → à appliquer en PROD** (lockstep : après push Abdou + déploiement Vercel, Claude applique via `backups/.db_password`, transaction atomique, vérif AVANT/APRÈS). **Additive/idempotente** (fonction + policies uniquement, aucune donnée modifiée). **Correctif PRIORITAIRE** : répare l'affichage des commandes de TOUS les acheteurs en prod.
