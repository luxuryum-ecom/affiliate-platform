...
- 🧱 **PROCHAINES BRIQUES — vision « SaaS où l'IA gère/corrige/valide, l'admin ne traite que les cas douteux »** : **BRIQUE 2** = contrôle qualité IA à la réception photo (rejeter floue/non-produit/interdite + prévenir le fournisseur). **BRIQUE 3** = chat conversationnel IA fournisseur (si prix manquant, le bot le DEMANDE dans Telegram au lieu de créer une fiche incomplète — requiert un état conversationnel, le bot est aujourd'hui stateless one-shot). **OPTION B** = inscription 100% Telegram (QR→bot→inscription conversationnelle, zéro formulaire web ; s'appuie sur BRIQUE 3).
