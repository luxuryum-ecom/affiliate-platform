#!/usr/bin/env bash
# Wrapper d'exécution des tests runtime migration 099 — RPC stock variante-aware + double-écriture.
...
echo "==> Exécution des tests migration 099 — RPC stock variante-aware + double-écriture..."
echo ""

LOCAL_SUPABASE_URL="$LOCAL_SUPABASE_URL" \
LOCAL_SERVICE_ROLE_KEY="$LOCAL_SERVICE_ROLE_KEY" \
LOCAL_ANON_KEY="$LOCAL_ANON_KEY" \
node "${SCRIPT_DIR}/test-variants-099-runtime.mjs"
