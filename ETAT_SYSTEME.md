...
- **Hook grossiste « économie totale par palier » (fiche détail) = EN PROD** (`23b2db8`, merge `370a276`) : sur `/wholesale/products/[id]`, met en avant l'ÉCONOMIE TOTALE (montant global) en achetant gros — `(prix_petit_palier − prix_palier) × quantité_palier` par palier + accroche avec le plus gros montant. **AFFICHAGE PUR** (lecture des `wholesale_tiers` stockés, zéro recalcul serveur, aucune donnée sensible) ; garde-fou < 2 paliers → rien ; séparation respectée (grossiste, zéro frais COD) ; i18n FR/AR/EN chiffres latins. `src/lib/wholesale-savings.ts` (helper pur + 8 tests) + `src/components/wholesale/wholesale-savings-hook.tsx`. Rendu FR vérifié runtime (ex. paliers 94/90/86/83 → 200/800/5 500). NB : cartes catalogue non couvertes (vue n'expose que `from_price`) — cadrage séparé si besoin.
