#!/usr/bin/env bash
# Wrapper d'exécution de la preuve Étape 7.C (migration 105).
...
# RÈGLE #8 : cible EXCLUSIVEMENT le Supabase local (127.0.0.1:54321).
set -euo pipefail
...
echo "==> URL locale: $LOCAL_SUPABASE_URL (GUARD: non-local = REFUSE)"
echo "==> Exécution preuve Étape 7.C — migration 105..."
echo ""

LOCAL_SUPABASE_URL="$LOCAL_SUPABASE_URL" \
LOCAL_SERVICE_ROLE_KEY="$LOCAL_SERVICE_ROLE_KEY" \
LOCAL_ANON_KEY="$LOCAL_ANON_KEY" \
node "${SCRIPT_DIR}/etape7-proof-local.mjs"
