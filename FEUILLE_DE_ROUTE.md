...
2. 🔴 **CHANTIER MONEY — purge `parseFloat` sur l'argent** (objectif Abdou « **zéro parseFloat sur l'argent nulle part** »). Découverte : surface bien plus large que les 7 sites d'`orders.ts` (~12 fichiers). Stratégie tranchée : **(A)** input → `parseMoneyInput` (string verbatim) ; **(B)** calcul → **centimes entiers** `Math.round(x*100)*qty/100` (validé @finance) ; **(C)** taux FX & marges % → helpers séparés `parseRateInput`/`parsePercentInput`, **lots dédiés distincts** (money.ts les corromprait) ; **(D)** preview client → dernier lot, sans @finance.
   ...
   - ✅ **LOT FX/% + COD FAIT + audité @finance GO (session 2026-06-14, décisions Abdou : half-up GO, taux 8 déc natif, % 0–100, COD parseMoneyInput serveur)** :
     ...
     - **🟠 GATE PRÉ-STAGING (réserve @finance, RÈGLE n°5)** : la borne **0–100 sur marge % (products.ts)** rejette désormais un produit existant à marge `percentage` **> 100 %** au re-save. **AVANT staging** : balayer la base `products WHERE platform_margin_type='percentage' AND platform_margin_value > 100` → corriger/migrer, sinon ces fiches deviennent non-éditables. (Décision Abdou = 0–100 assumée.)
   - 🔭 **RESTE money (2 sites, lots dédiés)** :
     - **🔴 `wholesale_tiers` — prix de palier B2B écrit CLIENT-AUTORITAIRE** : `product-form.tsx` `rowToTier` (`parseFloat` prix) → `JSON.stringify` champ caché → `products.ts:339` `JSON.parse` **stocké sans re-validation serveur**. = dette **money + sécurité** (un client peut POSTer n'importe quel prix). Lot dédié : validation serveur de chaque `price_per_unit` (parseMoneyInput) + envisager `price_per_unit` string. Circuit `@finance` + `@security`.
