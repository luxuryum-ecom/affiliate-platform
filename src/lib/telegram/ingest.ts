...
async function ingestProductMessage(admin: Admin, msg: TelegramMessage): Promise<void> {
  ...
  // Idempotence : l'insert staging (telegram_message_id UNIQUE) est le verrou.
  const { error: stageErr } = await admin.from('telegram_inbound').insert({
  ...
  if (stageErr) {
    if (stageErr.code === UNIQUE_VIOLATION) return // déjà traité — idempotent
  ...
  if (!supplierId) {
    await admin
      ...
      .update({ status: 'rejected', error: 'compte_non_lie', processed_at: nowIso() })
  ...
  try {
    ...
    await admin
      .from('telegram_inbound')
      .update({ status: 'failed', error: String(e instanceof Error ? e.message : e), processed_at: nowIso() })
      .eq('telegram_message_id', messageKey)
    await telegramSendMessage(
      chatId,
