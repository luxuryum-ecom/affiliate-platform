#!/usr/bin/env bash
# Wrapper d'exécution des tests runtime WMS-1.
...
if [ -z "$LOCAL_SUPABASE_URL" ] || [ -z "$LOCAL_SERVICE_ROLE_KEY" ] || [ -z "$LOCAL_ANON_KEY" ]; then
  echo "ERREUR: Impossible de récupérer les clés Supabase local."
  echo "Vérifiez que 'supabase start' est lancé."
  exit 1
fi

echo "==> URL locale: $LOCAL_SUPABASE_URL"
echo "==> Exécution des tests WMS-1..."
echo ""

LOCAL_SUPABASE_URL="$LOCAL_SUPABASE_URL" \
LOCAL_SERVICE_ROLE_KEY="$LOCAL_SERVICE_ROLE_KEY" \
LOCAL_ANON_KEY="$LOCAL_ANON_KEY" \
node "${SCRIPT_DIR}/test-wms1-stock-runtime.mjs"
