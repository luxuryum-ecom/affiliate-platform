...
- ⬜ **(3) RAIL CATÉGORIES SUR LE MARKETPLACE GROSSISTE GÉNÉRAL.** Constat : `CategoryRail` existe sur `/affiliate/products` et `/wholesale/products` mais **MANQUE sur `/wholesale/marketplace`** (seulement un dropdown). À ajouter en **réutilisant `CategoryRail` + `CATEGORY_ICONS` existants**. **Affichage pur, zéro argent.**
