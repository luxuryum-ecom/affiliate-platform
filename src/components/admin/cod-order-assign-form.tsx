...
import { assignCodOrder } from '@/app/actions/orders'

type Member = { id: string; name: string }

/**
 * LOT 1F — Assignation / réassignation 1 clic d'une commande COD à un agent.
 * Calque exact du patron B2B WholesaleOrderAssignForm. Appelle la server action
 * assignCodOrder (gardée par le casier `assign_orders`, admin court-circuité).
 * RTL piloté globalement par le `dir` du layout (classes Tailwind symétriques).
 */
export function CodOrderAssignForm({
  orderId,
  members,
  currentAgentId,
}: {
  ...
  const t    = useTranslations('admin.codAssign')
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const result = await assignCodOrder(orderId, selected)
