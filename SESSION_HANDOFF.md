...
> **Branche prod :** `main` **poussée sur `origin` @ `e842363` — DÉPLOYÉE Vercel (Ready) le 2026-07-04 soir**. Rien en attente de push. `origin` = prod live.
...
> **✅ SESSION 2026-07-05 = AUDIT RLS CIBLÉ + CORRECTIF FUITES INTER-ACTEURS** (branche `fix/rls-secrets-financiers`, **NON mergé/poussé** — GO Abdou requis). Audit @security = 6 findings. **Fermés** (mig `115`, @finance🟢 @security🟢, 4 checks verts / 591 tests) : **C1** prix source USD fournisseur (`supplier_product_moq_tiers.unit_price_usd`) lisible par tout grossiste → policy DROP + vue redacted + marketplace repointée ; **C2** idem variants (`price_adjustment_usd`) → policy DROP ; **E2** `products` SELECT base → staff-only (reprise mig 091). **⚠️ Migration 115 À LANCER EN PROD par Abdou en SQL Editor, APRÈS déploiement Vercel du code (lockstep gravé en tête).** **⏸️ Reportés** (fuites sur SES PROPRES lignes, entremêlées aux writes — règle #3) : **E1** (acheteur voit marge sur sa commande) + **M1** (fournisseur voit marge sur sa fiche) → lot dédié @finance ; **M2** → backlog. Diagnostic prod optionnel : `scripts/sql-manuels/RLS_secrets_diagnostic.sql` (lecture seule).
> **Migrations prod :** 001→**114** appliquées ✅ (114 = état `awaiting='unit'` + `proposed_unit`, appliquée en SQL Editor par Abdou le 2026-07-04, Success). ✅ **112 + 113 + 114 APPLIQUÉES.** ⚠️ **111 (fix données, non urgent) + 091** toujours en attente (SQL Editor, PAS `db push`).
