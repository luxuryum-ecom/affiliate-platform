...
> **✅ LES 4 LOTS MERGÉS `main` `--no-ff` (NON poussés). 4 checks verts sur `main` combiné (Lots 1+2+3+4) : tsc 0 · build · vitest 638 · smoke 16.** Détail par lot : mémoire `project-4lots-awaiting-go`. **⚠️ Reste : (1) Abdou pousse ; (2) après déploiement Vercel, Claude applique la migration 117 en prod (lockstep) ; (3) renseigner env vendeur facture `INVOICE_SELLER_*` + `INVOICE_VAT_RATE` dans Vercel avant vraies factures.**
>
