/**
 * Config Playwright dédiée aux tests runtime V5-bis (C2 fraîcheur 3 paliers + C4 dispo séparée).
 *
 * GARDE-FOU RÈGLE #8 : ce spec NAVIGUE sur des pages liées à des données seedées en LOCAL.
 * Le serveur de test est forcé sur le Supabase LOCAL (identifiants lus via « supabase status »),
 * JAMAIS sur .env.local / la prod.
 *
 * Pré-requis : « supabase start » + « node scripts/seed-c2c4-test-local.mjs --seed »
 */
...
const PORT = 3200 // port dédié V5-bis — n'entre pas en collision avec 3000/3100
const BASE_URL = `http://localhost:${PORT}`

export default defineConfig({
  testDir: './e2e',
  testMatch: /v5bis-c2c4\.spec\.ts/,
  fullyParallel: false,
  forbidOnly: false,
  retries: 0,
  workers: 1,
  reporter: [['list'], ['html', { open: 'never', outputFolder: 'playwright-report-v5bis' }]],
  timeout: 120_000,
