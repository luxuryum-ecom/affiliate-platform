...
> - ✅ P0 CŒUR ARGENT EXPOSÉ — MERGÉ `main` `--no-ff` + POUSSÉ (Vercel 04d5172) ; ✅ mig 125 APPLIQUÉE EN PROD 2026-07-10 (pooler, lockstep, vérif AVANT/APRÈS, rempart staff prouvé live, historique 001→125) : écran réconciliation livreur `/admin/remittances` (versement → commission auto-approuvée N1, créance partielle chiffrée) + cockpit trésorerie `/admin/treasury` (KPIs + soldes grand livre). Mig 125 (2 vues staff-only) appliquée LOCAL, PROD après GO. @finance 🟢 + @security 🟡 (P2 corrigés). Preuve 6/6, 4 checks verts, captures FR/AR/EN. + P2 bloc stats affilié dans `users/[id]`. Détail : `LIVRABLE_P0_DASHBOARDS.md`.
...
- ✅ **ONBOARDING FOURNISSEUR 1-CLIC** (`/pending`) — gros bouton **« Activer sur Telegram »** sur l'écran d'attente post-inscription (seul écran atteignable avant approbation, le dashboard fournisseur étant gaté `status='approved'`). Code émis/réutilisé **au rendu** via `ensureSupplierTelegramCode()` (extraction render-friendly de `generateTelegramLinkCode`, délégation → zéro duplication ; anti-churn = 1 code/fenêtre TTL 30 min ; gate `role='supplier'`, pas le statut → liaison possible avant approbation). Deep-link `t.me/<bot>?start=<code>` → 1 clic → « Démarrer » → lié (handler `/start CODE` déjà existant, non modifié). Visible **fournisseur non lié uniquement** (lié → pastille « ✅ connecté » ; grossiste/affilié → rien). i18n FR/AR/EN + RTL. **Sans migration, sans changement financier, `ingest.ts`/sécurité du code intacts.** @tester 6/6 intégration LOCAL · @security 🟢 (1 note MINEUR/INFO assumée : émission au rendu GET, bornée idempotent+anti-churn+RLS `auth.uid()`+session — choix conservé pour le vrai 1-clic) · 4 checks verts (tsc, build, 490 tests, smoke 16/4-skip).
- ✅ **MESSAGES BOT TELEGRAM — 4 LANGUES, ERREURS GUIDANTES** (`26f2c68`) — les 13 messages du bot fournisseur (liaison, garde-fous, accusé produit, guidage) routés FR/EN/AR-fus'ha/AR-darija via le MÊME `pickWelcomeLang` que l'accueil Lot 5. Erreurs **guidantes** (finissent par l'action + emoji). Module pur `src/lib/telegram/messages.ts` ; `ingest.ts` = textes seuls (pipeline/logique/sécurité inchangés). 26 tests unitaires. **Sans migration, sans changement financier.**
- ✅ **CORRECTIF DEVISE PALIERS EN MODÉRATION** (`f93333a`) — la fiche `/admin/supplier-products/[id]` codait « USD » en dur ; désormais `source_currency` (Maroc→MAD, UAE→AED, international→USD). **AFFICHAGE uniquement** (`extract.ts`/facturation/miroir NON touchés). @tester 3/3 LOCAL (MAD/AED/USD) + capture. Voir DETTE `unit_price_usd` mal nommée (section Dettes techniques).
- 🧱 **BRIQUES — vision « SaaS où l'IA gère/corrige/valide, l'admin ne traite que les cas douteux »** :
  - ✅ **BRIQUE 3 — chat conversationnel IA fournisseur** (branche `feat/telegram-conversational-bot`, mig 113) : produit incomplet (prix manquant / prix sans paliers) → le bot DEMANDE l'info manquante dans la langue du fournisseur (4 langues), la réponse texte complète le produit → arrive COMPLET en modération. État conversationnel = table `telegram_pending_products` (RLS deny, scopée supplier_id), relance one-shot ~1h via cron. @tester 8/8 + 16 unitaires, @security 🟢. **⚙️ Déploiement : appliquer mig 113 en prod (SQL Editor) + `CRON_SECRET` + Vercel Cron horaire sur `/api/telegram/reminders`.**
  ...
  - ✅ **UPGRADE PREMIUM ADMIN = EXISTANT** : `/admin/premium` → `assignPlan` (`src/app/actions/premium.ts:107`, `requireAdmin`) attribue un plan **sans paiement** (Gratuit=5 / Professionnel=50 / Entreprise=illimité via `premium_plans.max_products`, 0=illimité), `expires_at` vide = permanent, trace `subscription_audit_log`. **Reste à cadrer** : la suppression d'un produit libère-t-elle le quota (`checkProductLimit` compte `archived_at IS NULL`) ?
...
**✅ Chantier paliers Telegram CLOS** (Lots 1→5) — **TOUT EN PROD** (`origin/main` @ `7eba3d6`, déployé Vercel 2026-07-02).

---

## ✅ FAIT EN PROD — 2026-07-02 (mergés + poussés + déployés Vercel)
...
- **4 bloquants go-live** : rotation secrets (`SUPABASE_SERVICE_ROLE_KEY` + mdp admin) · backups auto prod (plan Pro/PITR + `.db_password`) · migration 091 · redirect `/auth/callback`.
...
- 🟠 **BLOQUANT GO-LIVE PUBLIC #1 — BACKUPS AUTO PROD — 🔧 RÉPARÉ CÔTÉ SCRIPT (2026-07-02), preuve à produire.** L'échec `SSL SYSCALL EOF` (depuis 2026-06-29) venait du dump via l'endpoint **DIRECT** (`supabase db dump --linked`) déprécié par Supabase. **Corrigé dans `backup-prod.sh`** : bascule sur le **pooler session-mode** (`--db-url` → `aws-1-eu-central-1.pooler.supabase.com:5432`), mdp lu de `~/AI-FACTORY/backups/.db_password` (mode 600, run auto lundi sans trousseau, mdp masqué des logs). Dernier bon dump **06-26 sécurisé en triple** (`_safe/` + iCloud). **RESTE :** créer `.db_password` → lancer le dump pooler 07-02 → **test de restauration LOCAL** (la vraie preuve, jamais faite). Le LaunchAgent reste PC-dépendant → **PITR (plan Pro) recommandé à terme** pour un filet serveur indépendant.
...
- ⬜ **DETTE — colonne `supplier_product_moq_tiers.unit_price_usd` MAL NOMMÉE** : elle stocke le prix **verbatim en devise SOURCE du fournisseur** (Maroc→MAD, UAE→AED…), **PAS forcément USD**. Même chose pour `supplier_products.supplier_unit_price_usd`. L'affichage a été corrigé (session 2026-07-03(c) : on affiche `source_currency`), mais le **nom de colonne reste trompeur** → à **renommer un jour** (`unit_price_source` / migration + repointage des reads) pour éviter toute confusion future (risque de re-régression « USD en dur »). Non bloquant.
- ⬜ **DETTE — SUPPRESSION DE COMPTE BLOQUÉE (impact RGPD + nettoyage comptes test)** — découvert 2026-07-03 par @tester. `supabase.auth.admin.deleteUser()` (et tout `DELETE FROM auth.users`) **échoue pour TOUT compte qui s'est connecté au moins une fois**. Cause : un trigger consigne un event `login` dans **`admin_audit_log`** (`actor_id` = le compte) ; cette table est **append-only** (`admin_audit_log_immutable` bloque UPDATE/DELETE), mais sa **FK vers `auth.users` est `ON DELETE SET NULL`** → le `SET NULL` implicite déclenché par le DELETE est **bloqué par le trigger d'immutabilité** (`ERROR: admin_audit_log est append-only`). **Impact** : toute suppression de compte (droit à l'effacement **RGPD** / offboarding / nettoyage des comptes de test) est **impossible**. **NON urgent** (pas de suppression en prod aujourd'hui) mais **à CADRER AVANT go-live Europe**. Touche **suppression de données + table financière/sécurité protégée** → circuit **`@backend-db` + `@security`** obligatoire (piste : FK `ON DELETE CASCADE` vers l'audit, ou conserver la ligne d'audit en détachant l'acteur autrement que par UPDATE — décision à trancher, la table est délibérément immuable). **Ne PAS corriger à la légère.**
...
- 🛡️ **Dette (résiduelle, non bloquante) — race condition bootstrap admin** : `promoteToAdmin` (`src/app/bootstrap/actions.ts`) est désormais **fail-closed** (refuse si un admin existe déjà ou si le comptage échoue — faille d'escalade fermée, auditée @security GO le 2026-06-29, prouvée e2e). **Reste** : deux POST simultanés alors que `admin_count = 0` pourraient créer 2 admins (chacun met à jour son propre `id`). Acceptable pour un bootstrap one-time `localhost`-only. **Durcissement optionnel** : RPC `bootstrap_first_admin()` SECURITY DEFINER faisant `count + update` sous `pg_advisory_xact_lock` (atomique). Ne PAS poser d'index unique partiel `WHERE role='admin'` (interdirait à jamais le multi-admin, non voulu).
...
- ⬜ **🏬 GESTION DÉPÔT & NOTIFICATIONS — chantier maître (créé 2026-06-29).** Détail complet + fichiers de référence dans **`ROADMAP_MASTER.md`** (section « GESTION DÉPÔT & NOTIFICATIONS »). Découpé en **3 lots**, **rien codé** :
  - **LOT 1 — BETA (essentiel)** : notif **COD affilié** sur 2 canaux (**Telegram + in-app**, le COD ne notifie rien aujourd'hui) ; **cloche in-app 🔔** modèle Shopify (table `notifications` existe, **UI manque**) ; **casiers de responsabilité** par compte dépôt (Réception/Emballage/Expédition/Confirmation/Supervision, provisoires & réversibles, **Option A via `team_members.permissions` existant**) ; **« qui traite quoi »** = responsable affiché + réassignable en 1 clic ; **destinataires cloche à compléter par Abdou**. ⚠️ notif COD = **PII → audit `@security`**.
    - → **✅ LOT 1F — assignation des commandes COD FAIT (2026-06-30)** : colonne `orders.assigned_to`/`assigned_at` + RPC `assign_cod_order_atomic` (mig **110**, calque B2B `assign_wholesale_order_atomic`) + extension du trigger d'audit `audit_sensitive_change` → action `order_assign_agent` sur `orders`. Server action `assignCodOrder` (orders.ts) **gardée `requireCapability('assign_orders')`** (admin court-circuité), RPC re-garde côté DB via `can_assign_orders`. UI : `cod-order-assign-form.tsx` (clone B2B) câblé dans la page détail COD `[id]/page.tsx` (affiche « Traitée par » + form assigner/réassigner 1 clic + liste agents). i18n `admin.codAssign` FR/AR(فصحى)/EN + RTL. **Assignation ORTHOGONALE au statut** (zéro colonne financière/statut touchée). **Décision Abdou respectée** : déléguer le casier reste admin-only (mig 107 inchangée) ; un agent avec `assign_orders` exécute mais ne distribue jamais. **@security 🟢** (double garde app+DB, protection PII assignee role∈agent/admin, audit inviolable par trigger, RLS `orders` inchangée) · **@tester tout vert** sur LOCAL (`tests/lot1f-assign-cod.integration.test.ts` : admin assigne→tracé ; agent AVEC casier→OK+tracé ; agent SANS casier→REFUSÉ `errors.forbidden_assign_orders` ; assignee non-agent→REFUSÉ `errors.assignee_not_found` ; réassignation + idempotence ; **331 tests, 0 régression**). **4 checks verts**. Mergé `main` `--no-ff`. ⚠️ **Migration 110 = LOCAL uniquement — `supabase db push` prod = en attente GO Abdou → feature INERTE en prod tant que `assigned_to` n'existe pas en Remote.**
    - → **✅ LOT 1B — notif COD affilié FAIT (2026-06-30)** : helper best-effort `src/lib/notifications/order-created.ts` (calque B2B `order-assigned`) + migration **109** (`notifications.cod_order_id`, index idempotence `uniq_notif_cod_event_recipient`, trigger audit AFTER INSERT `cod_order_created`). Déclenché **post-commit** dans `placeOrder`/`createAffiliateOrder` (création) + `confirmOrderAsSupervisor` (confirmation). Destinataires : admins + **l'affilié concerné UNIQUEMENT** + dépôt à casier COD (`confirm_cod_orders`/`depot_supervision`) ; payload **sans PII** (`{ref,items,city}`) ; Telegram **admin-only**. **@finance 🟢** (money.ts intact, zéro calcul) · **@security 🟢** (zéro PII, isolation affilié prouvée, RLS recipient-own intacte) · **@tester 7/7** sur LOCAL (`tests/lot1b-notif-cod.integration.test.ts` — mauvais affilié reçoit 0 notif, audit tracé, idempotence OK). **4 checks verts** (tsc 0 / build / vitest 325 / smoke 16+4skip). Mergé `main` `--no-ff`. ⚠️ **Migration 109 appliquée LOCAL uniquement — `supabase db push` prod = en attente GO Abdou → feature INERTE en prod tant que `cod_order_id` n'existe pas en Remote (best-effort absorbe).** Dette légère : étendre le mock `makeClient` de `tests/orders.test.ts` pour couvrir `notifyOrderCreated` (stderr best-effort, zéro test rouge).
  - **LOT 2 — TRAÇABILITÉ SCANS (post-beta)** : scan **téléphone** (QR/code-barre) flexible ; **4 scans nominatifs + horodatés + ineffaçables (append-only)** = réception/emballage/sortie/retour ; chaque action **exige la capacité (casier)** correspondante ; anti-perte/vol. Croise la vision **WMS** (voir section « WMS — Traçabilité stock par scan QR »).
  - **LOT 3 — MULTI-DÉPÔTS & ABSENCE (quand Dubaï arrive)** : **concept dépôt/responsable par pays** (inexistant, juste `countries.has_warehouse`) ; **push mobile** App Store/Play Store (Firebase/APNs) à la sortie de l'app ; **backup/remplaçant auto** en cas d'absence ; **réassignation en masse** d'un portefeuille.
  - **Contexte métier** : 5 personnes polyvalentes au dépôt **Casa** (rôles interchangeables) ; **Dubaï** bientôt ; scans par **téléphone** ; notifs visibles **façon Shopify/Deliveroo**.
...
## 🚀 POST-BETA — idées d'Abdou (NON BLOQUANTES, à cadrer plus tard)
> Consigné le **2026-06-30**. **Rien n'est codé.** Ajout pur, aucune suppression. À cadrer un à la fois après l'ouverture beta. Ces idées ne bloquent PAS le go-live.

- ⬜ **(PB-1) CHAT TELEGRAM DE RELANCE FOURNISSEUR (dialogue conversationnel multi-tours).** Aujourd'hui l'IA d'ingestion fait **UNE seule passe** et l'admin complète les trous à la modération. **Vision Abdou** : un **vrai dialogue conversationnel** où l'IA **détecte ce qui manque** et **RELANCE le fournisseur dans le bot** — « il manque le prix, envoie-le », « il manque le palier/seuil, donne-le », etc. — **jusqu'à compléter la fiche** (famille / sous-famille / unité de mesure / taille…). **Nouveau chantier de développement (multi-tours), à cadrer.** Recoupe (1) « SAISIE DES PALIERS FOURNISSEUR VIA TELEGRAM » et (2) « HOMOGÉNÉITÉ TELEGRAM » de la section BACKLOG 2026-06-20. ⚠️ S'il touche au prix/paliers → audit `@finance`.
- ⬜ **(PB-2) DEVISES DE SAISIE — décision Abdou GRAVÉE.** Devise de saisie dérivée du **pays du fournisseur** : **Maroc → MAD**, **Dubaï/EAU → AED**, **Turquie → USD**, **Égypte → USD**, **tout autre pays → USD par défaut**. **Seulement 3 devises de saisie : MAD, AED, USD.** **PAS** de livre turque ni de livre égyptienne. (Devise pivot interne reste **MAD**.) Lié au fix (PB-5) ci-dessous.
- ⬜ **(PB-3) CHANTIER « MANNEQUIN » — production interne, génération d'image IA.** Si un fournisseur envoie une **photo non-pro** (produit au sol / pris au téléphone), un **outil IA régénère le produit porté par un mannequin** / bien présenté pour le catalogue. Si la **photo est déjà présentable → bypass**. **Coût par image** à évaluer (**Higgsfield / Flux / Kling**). **Chantier séparé.**
- ⬜ **(PB-4) CHANTIER « CRÉAS PAYANTES AFFILIÉ/GROSSISTE » — monétisation à la carte, indépendant.** Les **affiliés/grossistes paient à la carte** pour générer des **visuels/vidéos marketing** de produit (**revenu passif**). **⚠️ TOUCHE L'ARGENT → circuit `@finance` + `@security` OBLIGATOIRE.** À cadrer : **paiement, quotas, stockage, modération**. **Gros chantier séparé.**
- ⬜ **(PB-5) FIX G4 (petit, post-test).** Lever le **hardcode Maroc / `local_stock`** dans `ingest.ts` → **dériver pays/type depuis le `country_code` du fournisseur**, pour un **bon étiquetage international**. **Non bloquant pour la devise.** Cohérent avec la décision devises (PB-2).
- 📝 **(PB-6) NOTE — pipeline n8n externe ABANDONNÉ.** L'**ingestion Telegram + IA tourne déjà nativement** dans la plateforme **via Claude**. Pas de dépendance n8n externe.
- ⬜ **(PB-7) TAUX DE CHANGE — pas d'écran admin de saisie/MAJ.** Constat (vérifié en prod, 2026-06-30) : les taux `current_exchange_rates` (**USD→MAD = 10**, **AED→MAD = 2.72**, EUR = 10.8) sont des **valeurs seed INDICATIVES** (`source = 'seed:indicative'`, posées le 2026-06-10), **jamais réactualisées**. La table `exchange_rates` est append-only et l'action serveur `upsertExchangeRate` (`src/app/actions/fx.ts`) existe **mais n'est branchée à AUCUNE page** (elle `revalidatePath('/admin/settings')` → route inexistante). **Conséquence** : impossible pour l'admin d'ajuster un taux central depuis l'UI (seul recours actuel = override par produit dans `product-form.tsx`). **Avant de vraies ventes internationales**, prévoir : **(a)** une **page admin de saisie/MAJ des taux figés** (câbler l'action `upsertExchangeRate` existante + créer la route `/admin/settings` ou équivalent), **OU (b)** un **rafraîchissement depuis une source marché** (API FX). **Petit fix code.** ⚠️ Taux = base de conversion des prix → **circuit `@finance`** au moment du chantier.

### 🏭 Gestion fournisseurs — 3 chantiers (par priorité)

- ⬜ **(PB-8) VRAI BAN FOURNISSEUR — PRIORITÉ HAUTE (vrai trou).** Aujourd'hui l'admin ne peut que **Approuver / Rejeter** (`updateUserStatus`, `/admin/users`) — statuts applicatifs limités à `pending/approved/rejected` (pas de `banned`/`suspended`). Rejeter coupe l'accès à l'espace supplier (via `(supplier)/layout.tsx`), **MAIS ⚠️ les produits du fournisseur banni RESTENT VISIBLES au catalogue** : les vues catalogue filtrent sur `approval_status='approved' + archived_at IS NULL` **sans jointure sur `profiles.status`** → aujourd'hui il faut archiver les produits **à la main** (ce que la mig one-shot `103` a fait manuellement). **À construire : (a)** un statut **`suspended`/`banned` RÉVERSIBLE** distinct du rejet + **bouton admin** (bannir / réactiver) ; **(b) surtout — MASQUAGE AUTOMATIQUE des produits d'un fournisseur banni du catalogue** (jointure sur le statut fournisseur dans les vues catalogue, OU archivage/désactivation en cascade réversible). C'est le vrai manque sécurité produit. i18n FR/AR/EN + RTL sur les libellés. ⚠️ Touche la visibilité catalogue → revue `@security`.
- ⬜ **(PB-9) CRUD PLANS ADMIN.** Le moteur de plans existe en base (mig 038 : `premium_plans`, `supplier_subscriptions`) et `/admin/premium` sait **assigner/résilier** un plan — mais il n'existe **AUCUNE UI pour créer / modifier / supprimer un plan** (prix `price_mad_monthly`, quota `max_products`, badges `featured_badge`/`verified_badge`, `rfq_priority_boost`). Toute modif de tarif/quota passe aujourd'hui par **SQL direct**. **À construire :** page admin **CRUD des plans** (`premium_plans`) sans SQL + **brancher les 2 flags aujourd'hui inertes** `full_analytics` et `priority_support` (colonnes présentes en base, aucun enforcement dans le code). i18n FR/AR/EN + RTL. ⚠️ Modifie des tarifs → sensibilité argent, revue `@finance` sur l'écran de prix.
- ⬜ **(PB-10) FACTURATION AUTO — GROS CHANTIER, ⚠️ ARGENT → `@finance` + `@security` OBLIGATOIRE.** Aujourd'hui **100 % manuel** : attribution des plans via `/admin/premium`, **zéro Stripe / zéro webhook paiement** (`grep stripe` = 0), aucun cron/edge function (`supabase/functions/` n'existe pas), et **même l'action d'expiration évoquée dans la mig 038 n'est pas codée** — seul effet auto = retombée en `free` **à la lecture** quand `expires_at <= now()`. **À construire : (a)** intégration **Stripe** (abonnement récurrent) ; **(b) webhooks paiement** (signés, idempotents, ledger append-only) ; **(c) blocage/downgrade AUTO pour impayé** ; **(d) cron d'expiration** des abonnements. **Circuit financier complet `@finance` + `@security` + validation Abdou AVANT tout commit.** Gros chantier séparé, à cadrer.
