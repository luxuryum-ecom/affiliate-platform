...
> **Branche prod :** `main` **en avance sur `origin` @ `df8b1ce` (merge lot RLS C1/C2/E2 — NON poussé, Abdou pousse lui-même)**. `origin` @ `e842363` déployé Vercel (Ready). Après push : ⚠️ **lancer migration 115 en SQL Editor APRÈS le déploiement (lockstep)**.
