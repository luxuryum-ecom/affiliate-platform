...
2. 🔴 **CHANTIER MONEY — purge `parseFloat` sur l'argent** (objectif Abdou « **zéro parseFloat sur l'argent nulle part** »). Découverte : surface bien plus large que les 7 sites d'`orders.ts` (~12 fichiers). Stratégie tranchée : **(A)** input → `parseMoneyInput` (string verbatim) ; **(B)** calcul → **centimes entiers** `Math.round(x*100)*qty/100` (validé @finance) ; **(C)** taux FX & marges % → helpers séparés `parseRateInput`/`parsePercentInput`, **lots dédiés distincts** (money.ts les corromprait) ; **(D)** preview client → dernier lot, sans @finance.
   ...
   - ⏳ **RESTE** (découpage affiné) :
     - **M8 preview client — ⚠️ RECLASSIFIÉ (vérif lecture seule 2026-06-14, 2 sites sur 3 mal étiquetés)** :
       - ✅ **GENUINE D (affichage, sans @finance)** : `product-form.tsx` (41/218-266/1269) = preview live de prix/commission, **jamais stocké**. MAIS entremêlé FX (`pp*er`) + % (`margin`/`pmValue`) → les lignes FX/% du preview doivent **miroiter** les lots FX/% différés ; seuls les `parseFloat` sur montants purs (factoryCostMad, sellPrice, fees) sont nettoyables ici, et nettoyer les `parseFloat((…).toFixed(2))` changerait le **nombre AFFICHÉ** de ±1 ct en bord (sans impact stockage). À cadrer.
       - 🔴 **PAS de l'affichage — FINANCIER** : `order-status-form.tsx:49` `parseFloat(codReceived)` → soumis à `updateOrderStatus` → **stocké dans `orders.cod_received`** (cash COD encaissé). = **changement financier COD** → circuit `@finance` + `@security-reviewer` + GO Abdou (règle absolue n°5), **PAS M8 sans @finance**.
       - 🔵 **PAS un montant — FX** : `admin/quote-requests/[id]/page.tsx:68` `parseFloat(r.rate_vs_mad)` = **taux de change** (`current_exchange_rates`) → **lot FX** (`parseRateInput`), pas M8.
     - **Lots FX/% dédiés** : `fx.ts` (22), `exchange_rate`/FX computations (products 124 gelé/86, quote-requests 107/130/266), marges % (products 103, supplier-products 184) → helpers `parseRateInput`/`parsePercentInput`. ⚠️ **Le lot FX porte la décision half-up `factory_cost` ci-dessus.**
   - ⚠️ Chaque lot money = `@finance` par site, validation Abdou si un calcul **change de résultat**. Cf. [[project-money-no-parsefloat]].
   ▶️ **REPRISE = arbitrage Abdou sur M8 reclassifié** : (1) `codReceived` COD → circuit finance complet ? (2) `product-form` preview pur affichage → nettoyer maintenant malgré le ±1 ct affiché en bord, ou aligner sur les helpers serveur au lot FX/% ? (3) `rate_vs_mad` → lot FX. Puis lots FX/% (half-up `factory_cost` + fees restants). M7a/M7b/M-products poussés.
