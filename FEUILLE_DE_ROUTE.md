...
- 🔄 **(3) RAIL CATÉGORIES SUR LE MARKETPLACE GROSSISTE GÉNÉRAL.** Constat : `CategoryRail` existe sur `/affiliate/products` et `/wholesale/products` mais **MANQUE sur `/wholesale/marketplace`** (seulement un dropdown). À ajouter en **réutilisant `CategoryRail` + `CATEGORY_ICONS` existants**. **Affichage pur, zéro argent.** → **🔄 CONSTRUIT sur `feat/navigation-marketplace`** (2026-06-21) : `CategoryRail` réutilisé tel quel, chips construits serveur (préservent les filtres en cours, ciblent le `?category=` déjà fonctionnel), inséré au-dessus du compteur de résultats. **Dropdown/filtres/grille intouchés** (diff = 66 ins / 1 del = ligne d'import). 4 checks verts. **En attente GO visuel Abdou (PHASE D) avant merge.**
- 🔄 **(4) PAGE D'ENTRÉE « GRANDES CARTES-IMAGES » PAR RAYON (vision UX forte).** Pour clients **analphabètes / petits commerçants** : page d'accueil catalogue avec de **GRANDES cartes visuelles par famille** (vraie image parlante : nourriture, viande, électrique…) → clic → produits de la niche. Sur le **marketplace général** (dispo + import). Réutilise `PRODUCT_CATEGORIES`, `resolveCategoryLabel`, i18n, drill-down niche. **MANQUE** : vraies **images par catégorie** (pas emojis) + composant `CategoryGrid`/`CategoryCard`. **Affichage pur, zéro argent.** → **🔄 CONSTRUIT sur `feat/navigation-marketplace`** (2026-06-21) : section **cartes-rayons EN HAUT** du marketplace (carrousel horizontal mobile / grille desktop, additif strict au-dessus de la grille produit) **+ route bonus `/wholesale/marketplace/categories`** (grille pleine). Composant `CategoryShowcase`/`CategoryCard` (Server Component pur, strings sérialisables, fallback emoji CSS). **12 vraies images self-hostées WebP** dans `public/categories/` (Unsplash libre de droit, validées Abdou, cf. `CREDITS.md`) + mapping `CATEGORY_IMAGES` dans `taxonomy.ts`. i18n FR/AR/EN + RTL vérifiés (captures). 4 checks verts. **En attente GO visuel Abdou (PHASE D) avant merge.**
  - **Décisions prises (validées Abdou en amont)** : cartes-rayons **EN HAUT** du marketplace (pas seulement route dédiée) ; images **self-hostées WebP optimisées** (robustesse/perf, pas de hotlink ni modif `next.config`) ; #2 Matières premières = blé doré (opt. A), #12 Autres = entrepôt (opt. A).
